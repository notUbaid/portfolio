...
export function usePhysicsPill(fallenClassName = '') {
  ...
  const handleClick = () => {
    ...
    if (pillRef.current) {
      ...
      // Add a +8px physical collision buffer so visual cards NEVER touch or overlap
      const bodyWidth = width + 8;
      const bodyHeight = height + 8;
      const randomPushX = (Math.random() - 0.5) * 1.2; 
      const randomAngularVel = (Math.random() - 0.5) * 0.02;
      
      const body = Matter.Bodies.rectangle(startX + width / 2, startY + height / 2, bodyWidth, bodyHeight, {
        restitution: 0.0,
        friction: 1.0,
        frictionStatic: 1.0,
        density: 0.01,
